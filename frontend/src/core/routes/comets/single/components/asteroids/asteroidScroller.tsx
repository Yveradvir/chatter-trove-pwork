...
import { AsteroidEntity, AsteroidFilters } from "@core/reducers/slices/asteroids/state";
...
const AsteroidScroller: React.FC<{ comet_id: number }> = ({ comet_id }) => {
    ...
    const { beReady, filter, page, loadingStatus, ids, entities, maxPages } = useAppSelector((state) => state.asteroids);
    ...
    const loadMoreAsteroids = () => {
        if (loadingStatus !== LoadingStatus.Loading && page < maxPages) {
        ...
        <div className="mt-4">
            ...
            <div className="mt-4">
                            ...
                            <div className="flex items-center gap-4">
                                <div className="flex-1">
                                    <Switchbu
                                        ...
                                        states={[
                                            {
                                                tooltip: { content: "Newest", placement: "top", children: <></> },
                                                content: <FiArrowDown />,
                                                field_value: "-",
                                            },
                                            {
                                                tooltip: { content: "Oldest", placement: "top", children: <></> },
                                                content: <FiArrowUp />,
                                                field_value: "",
                                            },
                                        ]}
                                ...
                                <div className="flex-1">
                                    <input
                                        ...
                                        className={clsx(
                                            ...
                                            touched.reply && errors.reply ? "border-red-500 ring-1 ring-red-500" : ""
                                        )}
                                    ...
                                    {touched.reply && errors.reply && (
                                        <p className="text-red-500 text-xs mt-1">{errors.reply}</p>
                                    )}
                                </div>
                                ...
                                <button
                                    ...
                                    onClick={() => {   
                                        const n_values = { 
                                            ...values, 
                                            reply: values.reply === '' ? null : values.reply 
                                        };
                                        
                                        dispatch(asteroidsActions.change_beReady({ new: false, comet: comet_id }));
                                        dispatch(asteroidsActions.change_filters(n_values));
                                        dispatch(asteroidsActions.change_beReady({ new: false, comet: comet_id }));
                                    }}                                   
                ...
                <InfiniteScroll
                    ...
                    hasMore={
                        loadingStatus !== LoadingStatus.Loading && page < maxPages
                    }
                ...
                >
                    <div className="mt-4 space-y-4">
                        {ids.map((id: EntityId) => {
                            ...
                            return <AsteroidCard key={id as string} asteroid={asteroid} />
                        })}
                    </div>
                </InfiniteScroll>
            </div>
