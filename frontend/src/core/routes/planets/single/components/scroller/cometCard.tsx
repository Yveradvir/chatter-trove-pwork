...
const CometCard: FC<{ comet: CometEntity; profile_view?: boolean }> = ({
    comet,
    profile_view = false,
}) => {
    ...
    const btn = () => {
        return (
            <button className="bottom-6 right-6 bg-gradient-to-t from-teal-500 to-emerald-600 hover:from-teal-600 hover:to-emerald-700 text-white font-bold py-3 px-4 rounded-full shadow-lg focus:outline-none focus:ring-2 focus:ring-blue-300">
    ...
    return (
        <div className="p-4 bg-neutral-800 rounded-lg shadow-md mb-4">
            ...
            <p className="text-sm text-neutral-400">
                {truncateDescription(comet.description)}
            </p>
            <Link
                to={
                    profile_view && comet.additionals.is_private
                        ? ""
                        : `/planets/${comet.planet}/comets/${comet.id}`
                }
            >
                {profile_view && comet.additionals.is_private ? (
                    <Tooltip
                        content="This comet belongs to the closed planet. You can't go there."
                        placement="top"
                    >
                        {btn()}
                    </Tooltip>
